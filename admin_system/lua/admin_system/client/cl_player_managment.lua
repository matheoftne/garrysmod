...
AdminSystem.Config.Buttons = {
    [0] = {
        ...
        onClick = function(self)
            ...
            banFrame = vgui.Create("AS_DFrame")
            ...
            banFrame.PaintOver = function(self, w, h)
                draw.SimpleText("Raison du bannissement", "Righteous25", Lib.RespX(25), Lib.RespY(98), color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
                draw.SimpleText("Durée du bannissement", "Righteous25", Lib.RespX(25), Lib.RespY(340), color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
                draw.SimpleText("(0 pour permanent)", "Righteous18", Lib.RespX(25), Lib.RespY(357), color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
            end

            local reasonEntry = vgui.Create("DTextEntry", banFrame)
            reasonEntry:SetSize(Lib.RespX(450), Lib.RespY(200))
            reasonEntry:SetPos(Lib.RespX(25), Lib.RespY(100))
            ...
            function reasonEntry:OnChange()
                ...
                if string.len(reason) > 2 && string.len(reason) < AdminSystem.Config.PlayerManagMaxLen then
            ...
            timeEntry:SetSize(Lib.RespX(350), Lib.RespY(60))
            timeEntry:SetPos(Lib.RespX(15), Lib.RespY(362))
            ...
            local timeOption = vgui.Create("AS_DComboBox", banFrame)
            ...
            local sendButton = vgui.Create("AS_DButton", banFrame)
            ...
            sendButton.DoClick = function(self)
                ...
                if timeEntry.IsSendable && reasonEntry.IsSendable then
                    ...
                    if IsValid(ply) && isnumber(time) && isstring(reason) && AdminSystem.ULXorFAdmin() then
                        LocalPlayer():ConCommand("ulx banid "..ply:SteamID().." "..tostring(time).." "..reason)
                    else
                        time = tostring(time)
                        LocalPlayer():ConCommand('FAdmin ban "'..ply:Nick()..'" '..time..' '..reason)
                    end
                    AdminSystem.AdminLogging(LocalPlayer(), "a banni "..ply:Nick().." pour "..reason.." pendant "..time.." minutes !")
                    ClosePanels()
                else
    ...
    [1] = {
        ...
        onClick = function(self)
            ...
            banFrame.PaintOver = function(self, w, h)
                draw.SimpleText("Raison du kick", "Righteous25", Lib.RespX(25), Lib.RespY(108), color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
            end

            local reasonEntry = vgui.Create("DTextEntry", banFrame)
            reasonEntry:SetSize(Lib.RespX(450), Lib.RespY(200))
            reasonEntry:SetPos(Lib.RespX(25), Lib.RespY(110))
            reasonEntry:SetMultiline(true)
            reasonEntry:SetFont("Righteous20")
            reasonEntry:SetValue("Non-respect des règles")
            reasonEntry.IsSendable = true
            ...
            sendButton.DoClick = function(s)
                ...
                if reasonEntry.IsSendable then
                    ...
                    AdminSystem.AdminLogging(LocalPlayer(), "a exclu "..ply:Nick().." pour "..reason.." !")
                    ClosePanels()
                else
    ...
    [9] = {
        ...
        onClick = function(self)
            ...
            if IsValid(ply) then
                local steamprofile = "https://steamcommunity.com/profiles/"..ply:SteamID64().."/"
...
function AdminSystem.OpenManagmentMenu()

    if !AdminSystem.Config.PlayerManagmentEnabled then return end
    if !IsValid(LocalPlayer()) then return end
	if !LocalPlayer():Alive() then return end
    if !AdminSystem.CheckStaff(LocalPlayer()) then return end

    basicFrame = vgui.Create("AS_DFrame")
	basicFrame:SetRSize(800, 850)
	basicFrame:Center()
	basicFrame:CloseButton(true)
    basicFrame.selectedPlayer = nil
	basicFrame:SetLibTitle(AdminSystem.Language.PlayerManagTitle.." de "..AdminSystem.Config.ServerName)

    local closeButton = vgui.Create("DImageButton", basicFrame)
    closeButton:SetSize(Lib.RespX(20), Lib.RespY(20))
    closeButton:SetPos(basicFrame:GetWide()-closeButton:GetWide()-Lib.RespX(5), Lib.RespY(5))
    closeButton:SetText("")
    ...
    leftPanel:SetSize(Lib.RespX(375), Lib.RespY(700))
    leftPanel:SetPos(Lib.RespX(400), Lib.RespY(100))
    ...
    rightPanel:SetSize(Lib.RespX(350), Lib.RespY(700))
    rightPanel:SetPos(Lib.RespX(25), Lib.RespY(100))
    ...
    for k,v in pairs(player.GetAll()) do
        ...
        plyButton:SetSize(Lib.RespX(350), Lib.RespY(45))
        if k > 1 then plyButton:SetPos(0, Lib.RespX(45)*(k-1)) end
        plyButton:SetText("")
        ...
        plyButton.DoClick = function(self)
            ...
            if v == basicFrame.selectedPlayer then
                ...
                infoFrame = vgui.Create("AS_DFrame")
                ...
                infoFrame.PaintOver = function(self, w, h)
                    ...
                    draw.SimpleText("Rang : "..rank, "Righteous30", w/2, Lib.RespY(560), color_white, 1, 1)
                    draw.SimpleText("Métier : "..job, "Righteous30", w/2, Lib.RespY(600), color_white, 1, 1)
                    draw.SimpleText("Argent : "..money.."€", "Righteous30", w/2, Lib.RespY(640), color_white, 1, 1)
                end
                ...
                playermodel:SetPos(0, Lib.RespY(50))
                ...
                for key,butt in pairs(AdminSystem.Config.Buttons) do

                    local commandButton = vgui.Create("AS_DButton", bscrollPanel)
...
net.Receive("AdminSystem:PlayerManag:Open", AdminSystem.OpenManagmentMenu)